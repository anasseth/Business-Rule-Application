  ...
  ConditionsBuilder.prototype = {
    init: function () {
      this.fields = this.denormalizeOperators(
        ...
        this.options.variable_type_operators
      );
      if (
        localStorage.getItem("conditions") != undefined &&
        localStorage.getItem("conditions") != null
      ) {
        this.data = JSON.parse(localStorage.getItem("conditions"));
      } else {
    ...
    buildConditional: function (ruleData) {
      ...
      select.append(
        $("<option>", { value: "all", text: "All", selected: kind == "all" })
      );
      select.append(
        $("<option>", { value: "any", text: "Any", selected: kind == "any" })
      );
      ...
      addConditionLink.click(function (e) {
        ...
        var f = _this.fields[0];
        var newField = {
          all: [{ name: f.value, operator: f.operators[0], value: null }],
        };
    ...
    buildRule: function (ruleData) {
      ...
      fieldSelect.change(
        onFieldSelectChanged.call(this, operatorSelect, ruleData)
      );
  ...
  function onOperatorSelectChange(e) {
    ...
    switch (option.data("field_type")) {
      ...
      case "select_multiple":
        ...
        var select = $(
          "<select class='value' multiple size='" +
            selectLength +
            "''></select>"
        );
        for (var i = 0; i < options.length; i++) {
